...
# Railway variables mein BOT_TOKEN aur ADMIN_ID set karna behtar hota hai
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8833268418:AAHE8Nj6XhNZU3md-XvvnjMngS2VvtuVzb0") 
ADMIN_ID = int(os.environ.get("ADMIN_ID", "7422190601"))
...
def start(message):
    ...
    if users[cid].get("strikes", 0) >= 3:
        bot.send_message(int(cid), f"🚫 **BANNED**\nContact @{config['support_user']}")
        return

    try:
        bot.send_photo(int(cid), config['banner_url'])
    except: pass 
    ...
    bot.send_message(int(cid), text_body, parse_mode="Markdown", reply_markup=main_menu())
